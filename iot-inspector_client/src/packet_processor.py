...
class PacketProcessor(object):
    ...
    def _process_packet_helper(self, pkt):
        ...
        ##BORRAR REGISTRE DEBUG
        #utils.log('[Packet Capture Helper] Process paquet.',pkt.src, "-->", pkt.dst)
        #utils.log('[Packet Capture Helper] Es ARP?.',sc.ARP)
        ###
        if sc.ARP in pkt:

            ##BORRAR REGISTRE DEBUG
            utils.log('[Packet Capture Helper] Process paquet ARP.',pkt.src, "-->", pkt.dst)
            ###
            return self._process_arp(pkt)

        if sc.DHCP in pkt:
            ##BORRAR REGISTRE DEBUG
            utils.log('[Packet Capture Helper] Process paquet DHCP.',pkt.src, "-->", pkt.dst)
            ###
        ...
        if sc.TCP in pkt and pkt[sc.TCP].flags == 'SA' and sc.IP in pkt:
            ##BORRAR REGISTRE DEBUG
            utils.log('[Packet Capture Helper] Process paquet SYN. SYN-ACK')
            #######
            tcp_layer = pkt[sc.TCP]
            if tcp_layer.dport == SYN_SCAN_SOURCE_PORT and tcp_layer.ack == SYN_SCAN_SEQ_NUM + 1:
                ##BORRAR REGISTRE DEBUG
                utils.log('[Packet Capture Helper] Process paquet resposta. SYN-ACK')
                #######
        ...
        if not (sc.Ether in pkt and sc.IP in pkt):
            ##BORRAR REGISTRE DEBUG
            utils.log('[Packet Capture Helper] Error . Packet desconegut . No es ni IP ni Eth')
            #utils.log('[Packet Capture Helper] Error :', pkt[sc.IP].src , "--> ",pkt[sc.IP].dst)
            #######
            return
        ...
        if utils.TEST_OUI_LIST:
            ##BORRAR REGISTRE DEBUG
            utils.log('[Packet Capture Helper] Process paquet OUI LIST')
            #######
        ...
        if self._host_state.host_ip in (pkt[sc.IP].src, pkt[sc.IP].dst):
            ...
            ##BORRAR REGISTRE DEBUG
            #utils.log('[Packet Capture Helper] Ignorem paquet amb origen o desti al BPIR64:',pkt[sc.IP].src ,"-->",pkt[sc.IP].dst)
            #######
        ...
        if sc.DNS in pkt:
            ##BORRAR REGISTRE DEBUG
            utils.log('[Packet Capture Helper] Procesat paquet DNS :',pkt[sc.IP].src ,"-->",pkt[sc.IP].dst)
            #######
        ...
        if sc.TCP in pkt:
            ##BORRAR REGISTRE DEBUG
            utils.log('[Packet Capture Helper] Procesat paquet TCP :',pkt[sc.IP].src ,"-->",pkt[sc.IP].dst)
            #######
            protocol = 'tcp'
        elif sc.UDP in pkt:
            ##BORRAR REGISTRE DEBUG
            utils.log('[Packet Capture Helper] Procesat paquet UDP :',pkt[sc.IP].src ,"-->",pkt[sc.IP].dst)
            #######
            protocol = 'udp'
        else:
            return
    ...
    def _process_tcp_udp_flow(self, pkt, protocol):
        ...
        #### ESBORRAR AL ACABAR
        utils.log('[PACKET PROCESSOR] Inici de Procesat TCP UDP.')
        #######
        ...
        gw_mac = self._host_state.gateway_mac
        ...
        # En funcio de si la mac desti es la del gateway
        if src_mac == gw_mac:         
            ##BORRAR REGISTRE DEBUG
            utils.log('[Packet Capture Helper] Processat paquet Flow ENTRANT : ',pkt[sc.IP].src, "(", pkt.src, ") -->", pkt[sc.IP].dst , "(", pkt.dst ,")")
            #####
            direction = 'inbound'
            device_mac = pkt.dst
            ...
            ### BORRAR REGISTRE DEBUG
            utils.log('[Packet Capture Helper] Processat paquet Flow ENTRANT : device_mac = ', device_mac , ', device_port = ', device_port , ', remote_ip = ', remote_ip, ', remote_port = ', remote_port)
            #####
        elif dst_mac == gw_mac:         
            ##BORRAR REGISTRE DEBUG
            utils.log('[Packet Capture Helper] Processat paquet Flow SORTINT : ',pkt[sc.IP].src, "(", pkt.src, ") -->", pkt[sc.IP].dst , "(", pkt.dst ,")")
            #####
            direction = 'outbound'
            device_mac = src_mac
            ...
            ### BORRAR REGISTRE DEBUG
            utils.log('[Packet Capture Helper] Processat paquet Flow SORTINT : device_mac = ', device_mac , ', device_port = ', device_port , ', remote_ip = ', remote_ip, ', remote_port = ', remote_port)
            #####
        else:
            ### BORRAR REGISTRE DEBUG
            utils.log('[Packet Capture Helper] Processat paquet Flow DESCONEGUT : ',pkt[sc.IP].src, "(", pkt.src, ") -->", pkt[sc.IP].dst , "(", pkt.dst ,")")
             #####
            return
        ...
        try:
            with self._host_state.lock:
                ...
                ### BORRAR REGISTRE DEBUG
                utils.log('[Packet Capture Helper] Processat paquet Flow : real_remote_device_mac = ', real_remote_device_mac )
                utils.log('[Packet Capture Helper] Processat paquet Flow : remote_device_id = ', remote_device_id )
                ##########
